...
softmax <- function(beta = 1, X)
  sapply(X, function(x) (exp(beta * x))/sum(exp(beta * X)))
```
...
agent_update_actions <- function(actions, state, parameters) {
  ...
  actions[actions_excel == 'r/2'] <- actions[actions_excel == 'r/2'] * prob_error
  actions[actions_excel == 'RR'] <- actions[actions_excel == 'RR'] * prob_error
  actions[actions_excel == 'RR/2'] <- actions[actions_excel == 'RR/2'] * prob_error
...
set.seed(123)
RUNS <- 10 # 10 runs: 27s runtime (Andreas, 20191205)
