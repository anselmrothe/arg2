...
outcome_chosen_features <- out_raw %>% 
  group_by( param_ok, run ) %>% 
  filter( i == max(i) ) %>% 
  ungroup %>% 
  group_by( param_ok ) %>% 
  summarize( both_long          = sum( slot_A == "long"  & slot_B == "long" ),
             only_rare_long     = sum( slot_A == "long"  & slot_B == "short" ),
             only_frequent_long = sum( slot_A == "short" & slot_B == "long" ),
             both_short         = sum( slot_A == "short" & slot_B == "short" ),
             rare_missing       = sum( slot_A == "[   ]" & slot_B %in% c("long", "short") ),
             frequent_missing   = sum( slot_A %in% c("long", "short") & slot_B == "[   ]" ),
             both_missing       = sum( slot_A == "[   ]" & slot_B == "[   ]" )
  )
```

```{r plot_outcome}
outcome_chosen_features %>% 
